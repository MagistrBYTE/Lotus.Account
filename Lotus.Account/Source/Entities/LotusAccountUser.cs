...
namespace Lotus
{
    namespace Account
    {
        //-------------------------------------------------------------------------------------------------------------
        /**
         * \defgroup AccountEntities Сущности предметной области
         * \ingroup Account
            ...
            public static void ModelCreating(ModelBuilder modelBuilder)
            {
                ...
                model.HasMany(user => user.FieldActivities)
                    ...
                    .UsingEntity<CUserFieldActivity>(
                        field => field.HasOne<CFieldActivity>()
                    .WithMany()
                        .HasForeignKey(x => x.FieldActivityId)
                        .IsRequired()
                        .OnDelete(DeleteBehavior.Restrict),
                        user => user.HasOne<CUser>()
                    .WithMany()
                        .HasForeignKey(x => x.UserId)
                        .IsRequired()
                        .OnDelete(DeleteBehavior.Restrict))
                    .HasKey(nameof(CUserFieldActivity.Id));

                model.HasMany(user => user.Groups)
                    .WithMany(group => group.Users)
                    .UsingEntity<CUserGroup>(
                        group => group.HasOne<CGroup>()
                    .WithMany()
                ...
                //	.WithMany(group => group.Users)
                //	.UsingEntity(j => j.ToTable("UserGroup", XDbConstants.SchemeName).HasKey("Id"));
